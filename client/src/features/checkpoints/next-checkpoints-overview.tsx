...
import { AlertTriangle, Calendar, Target, Clock, ChevronRight } from "lucide-react";
...
export default function NextCheckpointsOverview({
  ...
  const checkpointData = useMemo(() => {
    ...
    const overdue = enrichedCheckpoints
      .filter((cp) => cp.status !== "completed" && isBefore(cp.dueDate, now))
      .sort((a, b) => a.dueDate.getTime() - b.dueDate.getTime())
      .slice(0, 5);
  ...
  const getUrgencyBadge = (checkpoint: any) => {
    ...
    const daysUntilDue = Math.ceil(
      (checkpoint.dueDate.getTime() - nowSP().getTime()) / (1000 * 60 * 60 * 24)
    );
    if (isOverdue) return <Badge variant="destructive">Atrasado</Badge>;
    if (daysUntilDue <= 1) return <Badge variant="destructive">Vence hoje</Badge>;
    if (daysUntilDue <= 3)
      return (
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-1 flex-1">
            <div className="flex items-center gap-2">
              <h4 className="font-medium text-sm text-gray-900">{checkpoint.title}</h4>
              {getUrgencyBadge(checkpoint)}
            </div>
            <p className="text-xs text-gray-600 line-clamp-1">{checkpoint.keyResult?.title}</p>
          </div>
          ...
          <div className="w-full bg-gray-200 rounded-full h-1.5">
            <div
              ...
              style={{
                width: `${Math.min(
                  100,
                  ((parseFloat(checkpoint.actualValue) || 0) /
                    (parseFloat(checkpoint.targetValue) || 1)) *
                    100
                )}%`,
              }}
            />
    ...
    <div className="space-y-6">
      {checkpointData.overdue.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {checkpointData.overdue.length} marco
              {checkpointData.overdue.length !== 1 ? "s" : ""} com prazo vencido
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Marcos com vencimento nos próximos 7 dias</CardDescription>
        </CardHeader>
        <CardContent>
          {checkpointData.upcoming.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              <Target className="h-12 w-12 mx-auto mb-3 text-gray-300" />
              <p className="text-sm">Nenhum marco próximo</p>
              <p className="text-xs text-gray-400 mt-1">Todos os marcos estão em dia!</p>
            </div>
          ) : (
            <div className="space-y-3">
